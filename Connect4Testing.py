...
class Player:
    ...
    def next_move(self, board: Connect4):
        ...
        check_score = self._scores_for(board, self.player, self.ply)
    ...
    def _scores_for(self, board: Connect4, player, ply):
        ...
        scorred_columns = [1] * board.width # Creates a list to store values
        if player == "O": # Switches between AI when odd ply, even ply is opponent
            opponent = "X"
        else:
            opponent = "O"
            ...
            if board.is_legal_move(column) == True:
                ...
                if board.is_win_for(player) == True: 
                    ...
                    if ply > 1:                        
                        ...
                        if 0 in opponents_score:
                            scorred_columns[column] = 0
                        elif 100 in opponents_score:
                            scorred_columns[column] = 100
                        else:
...
def main():
    ...
    # Call _scores_for and print results
    scores = player._scores_for(board, "O", 3)
    print(f"Scores: {scores}") # Should be Scores: [50, 50, 100, 100, 50, 50, 50]
    nextmove = player.next_move(board)
    print(f"Next move is: {nextmove}")
