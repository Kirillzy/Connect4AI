...
class Connect4:
    ...
    def play_game_with(self, AI, GUI):
        pass
...
class GUI:
    def __init__(self, window, board: Connect4, ai: Player):
        self.window = window
        self.board = board
        self.ai = ai
        ...
        self.new_game_button = Button(self.frame,
                                      text="New Game!",
                                      command=self.new_game)
        # self.new_game_button.pack() # Currently doesnt work


        self.canvas = Canvas(self.frame, height=self.height, width=self.width, bg="blue")
        self.canvas.pack()

        # Draw the Grid
        self.create_grid()

        # Bind mouse clicks to add pieces
    ...
    def create_grid(self):
        '''Draw the 7x6 grid on the canvas'''
        for row in range(self.rows):
            for col in range(self.columns):
                ...
                # Draw the cell as an oval (circle)
                self.canvas.create_oval(x1 + 5, y1 + 5, x2 - 5, y2 - 5, fill="white")
    
    def handle_click(self, event):
        col = event.x // self.cell_size  # Determine which column was clicked
        print(event)
        self.handle_board(col)
        # Find the lowest empty slot in the column for the GUI
        for row in range(self.rows - 1, -1, -1):
            if self.grid[row][col] is None:  # Empty slot found
                self.grid[row][col] = self.current_player  # Update the grid
    ...
    def handle_AI(self):
        col = self.ai.next_move(self.board)
        self.board.add_move(col, "O")
        for row in range(self.rows - 1, -1, -1):
            if self.grid[row][col] is None:  # Empty slot found
                self.grid[row][col] = self.current_player  # Update the grid
    ...
    def handle_board(self, col):
        print(self.board)

        players_turn = "X" 
        player_move_col = col
        if self.board.is_legal_move(player_move_col) == True:
            self.board.add_move(player_move_col, "X")
            if self.board.is_win_for(players_turn) == True:
                print(players_turn + " has won!\n")
                self.winner_label = Label(self.window, text="X has won", font=("Arial", 14))
                self.winner_label.pack(side="top", pady=20)
    ...
    def new_game(self):
        '''Creates new game by recalling create grid and clear board'''
        self.board.clear()
        self.create_grid()
...
def main():
    board = Connect4(7,6)
    my_player = Player("O", "Random", 1)
    # board.play_game_with(my_player)
    window = Tk()
    window.title("Connect 4 against AI")

    my_window = GUI(window, board, my_player)
